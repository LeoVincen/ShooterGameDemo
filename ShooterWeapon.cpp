...
void AShooterWeapon::OnEquip(const AShooterWeapon* LastWeapon)
{
	...
	if (LastWeapon)
	{
		float Duration = PlayWeaponAnimation(EquipAnim, 1.0f);
		if (Duration <= 0.0f)
		{
			// failsafe
			Duration = 0.5f;
	...
	{
		PlayWeaponSound(EquipSound, 1.0f);
	}
...
void AShooterWeapon::StartReload(bool bFromReplication)
{
	...
	if (bFromReplication || CanReload())
	{
		...
		float AnimDuration = PlayWeaponAnimation(ReloadAnim, 1.0f);		
		...
		if (MyPawn && MyPawn->IsLocallyControlled())
		{
			PlayWeaponSound(ReloadSound, 1.0f);
		}
	}
}
...
void AShooterWeapon::HandleFiring()
{
	...
	else if (MyPawn && MyPawn->IsLocallyControlled())
	{
		...
		{
			PlayWeaponSound(OutOfAmmoSound, 1.0f);
...
bool AShooterWeapon::GetbWantToFire()
{
	return bWantsToFire;
}

bool AShooterWeapon::GetbPendingReload()
{
	return bPendingReload;
}

bool AShooterWeapon::GetbIsEquipped()
{
	return bIsEquipped;
}
...
void AShooterWeapon::SetTimeBetweenShots(float time)
{
	WeaponConfig.TimeBetweenShots = time;
}
...
UAudioComponent* AShooterWeapon::PlayWeaponSound(USoundCue* Sound, float PitchRate)
{
	...
	AC->SetPitchMultiplier(PitchRate);

	return AC;
}

float AShooterWeapon::PlayWeaponAnimation(const FWeaponAnim& Animation, float PlaySpeed)
{
	float Duration = 0.0f;
	if (MyPawn)
	{
...
void AShooterWeapon::SimulateWeaponFire()
{
	...
	{
		PlayWeaponAnimation(FireAnim, 0.2f / GetTimeBetweenShots());
	...
	if (bLoopedFireSound)
	{
		if (FireAC == NULL)
		{
			FireAC = PlayWeaponSound(FireLoopSound, 0.05f / GetTimeBetweenShots());
		}
	}
	else
	{
		PlayWeaponSound(FireSound, 1.0f);
	}
...
void AShooterWeapon::StopSimulatingWeaponFire()
{
	...
	if (FireAC)
	{
		...
		PlayWeaponSound(FireFinishSound, 1.0f);
	}
}
...
void AShooterWeapon::SetCurrentAmmo(int ammo) 
{
	CurrentAmmo = ammo;
}
...
void AShooterWeapon::SetCurrentAmmoInClip(int ammoInClip) 
{
...
void AShooterWeapon::SetAmmoPerClip(int ammoPerClip)
{
	WeaponConfig.AmmoPerClip = ammoPerClip;
}
...
void AShooterWeapon::SetMaxAmmo(int maxAmmo)
{
	WeaponConfig.MaxAmmo = maxAmmo;
}
