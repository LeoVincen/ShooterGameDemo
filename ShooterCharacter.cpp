...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	else
	{
		...
		PlayerInputComponent->BindAction("ImGui", IE_Pressed, this, &AShooterCharacter::OnImGuiPressed);
	}
...
void AShooterCharacter::UpdateImGuiBoard()
{
	ImGui::Begin("Test", nullptr, ImGuiWindowFlags_AlwaysAutoResize);

	//子弹总数
	static int ca = GetWeapon()->GetCurrentAmmo();
	static int last_ca = ca;
	ImGui::InputInt("CurrentAmmo", &ca);
	//开火和装弹时更新GM面板中的子弹数
	if (GetWeapon()->GetbWantToFire() || GetWeapon()->GetbPendingReload() || ImGuiRefresh)
	{
		ca = GetWeapon()->GetCurrentAmmo();
	}
	ca = FMath::Min(GetWeapon()->GetMaxAmmo(), ca);
	if (last_ca != ca)
	{
		GetWeapon()->SetCurrentAmmo(ca);
	}
	last_ca = ca;

	//弹夹里现存子弹
	static int caic = GetWeapon()->GetCurrentAmmoInClip();
	static int last_caic = caic;
	ImGui::InputInt("CurrentAmmoInClip", &caic);
	//开火和装弹时更新GM面板中的子弹数
	if (GetWeapon()->GetbWantToFire() || GetWeapon()->GetbPendingReload() || ImGuiRefresh)
	{
		caic = GetWeapon()->GetCurrentAmmoInClip();
	}
	caic = FMath::Min(GetWeapon()->GetAmmoPerClip(), caic);
	if (last_caic != caic)
	{
		GetWeapon()->SetCurrentAmmoInClip(caic);
	}
	last_caic = caic;

	//每个弹夹的子弹数
	static int apc0 = GetWeapon()->GetAmmoPerClip();
	ImGui::InputInt("AmmoPerClip", &apc0);
	if (GetWeapon()->GetAmmoPerClip() != apc0)
	{
		GetWeapon()->SetAmmoPerClip(apc0);
	}

	//最大子弹数
	static int ma0 = GetWeapon()->GetMaxAmmo();
	ImGui::InputInt("MaxAmmo", &ma0);
	if (GetWeapon()->GetMaxAmmo() != ma0)
	{
		GetWeapon()->SetMaxAmmo(ma0);
	}

	//生命值
	static int h0 = Health;
	ImGui::InputInt("Health", &h0);
	h0 = FMath::Min(GetMaxHealth(), h0);
	if (Health != h0)
	{
		Health = h0;
	}
	...
	//最大生命值
	static int mh = GetMaxHealth();
	ImGui::InputInt("MaxHealth", &mh);
	if (GetMaxHealth() != mh)
	{
		SetMaxHealth(mh);
	}
	...
	//射击间隔
	static float tbs = GetWeapon()->GetTimeBetweenShots();
	ImGui::InputFloat("TimeBetweenShots", &tbs, 0.0f, 1.0f, 2);
	if (GetWeapon()->GetTimeBetweenShots() != tbs)
	{
		GetWeapon()->SetTimeBetweenShots(tbs);
	}
...
void AShooterCharacter::OnImGuiPressed()
{
	ImGuiSwitch = !ImGuiSwitch;
	if (ImGuiSwitch) 
	{
		ImGuiRefresh = true;
	}
...
void AShooterCharacter::SetMaxHealth(int maxHealth) 
{
	GetClass()->GetDefaultObject<AShooterCharacter>()->Health = maxHealth;
}
